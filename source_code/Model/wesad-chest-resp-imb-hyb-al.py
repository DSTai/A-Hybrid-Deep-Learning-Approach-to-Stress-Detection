...
path = "/results/wesad/test-S" + str(subject) + "/10s/chest/RESP/bias_imb_hyb_al/hyb_imb_al" + \
             str(iter) + "_" + str(AL) + "_" + dtime  + ".txt"
...
# Add LSTM layers
model_m.add(layers.LSTM(hidden_units, activation='tanh', return_sequences=True))
model_m.add(layers.LSTM(hidden_units))
model_m.add(layers.Dropout(0.5))

# Add a Dense layer with softmax activation
model_m.add(layers.Dense(hidden_units, activation='relu'))
model_m.add(layers.Dense(dense_units, use_bias=True,
...
filepath="weights"+str(TIME_PERIODS)+str(STEP_DISTANCE)+str(EPOCHS)+str(BATCH_SIZE)+str(AL)+"_stress.best.hdf5"
...
with open(report_dir, "a") as text_file:
    ...
    for l in range(iter):
        ...
        model_m.fit(x_train, y_train, batch_size=BATCH_SIZE, epochs=EPOCHS, callbacks=callbacks_list, verbose=1)

        print("================Test phase==================: ", l)

        model_m.load_weights(
            "weights" + str(TIME_PERIODS) + str(STEP_DISTANCE) + str(EPOCHS) + str(BATCH_SIZE) +str(AL)+ "_stress.best.hdf5")
...
model_m.load_weights("weights"+str(TIME_PERIODS)+str(STEP_DISTANCE)+str(EPOCHS)+str(BATCH_SIZE)+str(AL)+"_stress.best.hdf5")
print("Loaded model from disk")
...
y_pred_test = model_m.predict_proba(x_test)
