...
def write_report_to_file(result, filePath, timePeriods, max_y_test, max_y_pred_test):
    ...
    rp_dir = os.path.join(project_root,  "Model", "reports", "result", "wesad_resp")
    #rp_dir = ("THESIS/Model/reports/S13b/result/wesad_resp")
...
def execute(TIME_PERIODS):
    ...
    model_m.add(layers.LSTM(128, activation='tanh', input_shape=(TIME_PERIODS, N_FEATURES)))
    ...
    # Hyper-parameters
    report_dir = os.path.join(project_root, "Model", "reports", "result", "wesad_resp", "S13-1")
    #report_dir = "THESIS/Model/reports/S13b"
    os.makedirs(report_dir, exist_ok=True)
    #WEIGHTS_PATH = os.path.join(project_root, "Model", "weights", "chest_resp")
    WEIGHTS_PATH = os.path.join(project_root, "Model", "WESAD", "test")
    #WEIGHTS_PATH = "THESIS/Model/weights/chest_resp/"
    ...
    filepath = os.path.join(WEIGHTS_PATH, f"weights" + str(TIME_PERIODS) + str(STEP_DISTANCE) + str(
    ...
    model_m.load_weights("weights" + str(TIME_PERIODS) + str(STEP_DISTANCE) + str(EPOCHS) + "_activity.best.hdf5")
    ...
    print(report)
    report_file_path = os.path.join(report_dir, f"chest_resp_batch_size{BATCH_SIZE}_epoch{EPOCHS}.txt")
