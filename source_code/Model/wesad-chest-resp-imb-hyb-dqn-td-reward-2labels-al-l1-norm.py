...
model_save_path = 'best_model_S'+str(subject)+'_dqn_td_al_'+str(AL)+'.hdf5'

path = "/results/wesad/test-S" + str(subject) + "/10s/chest/RESP/lstm_bias_imb_hyb_dqn_td_al_l1norm/reward-2labels/epochs" + str(EPOCHS) + "/lstm_imb_hyb_dqn_td_al_l1norm" + "eps_" + \
       str(EPISODES) + "_al_" + str(AL) + "_" + dtime + ".txt"
cwd = os.getcwd()
report_dir = cwd + path
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            if not done:
                ...
                print('self.gamma * self.model.predict(next_state)[0][action_next]', self.gamma * self.target_model.predict(state)[0][action_next])
...
filepath = "weights_dqn_td_al_" + str(AL) + "_best.hdf5"
...
with open(report_dir, "a") as text_file:
    ...
    for e in range(EPISODES):
        ...
        for l in range(iter_AL):

            print("===========Active Learning Phase==============:", l)

            text_file.write("===========Active Learning Phase==============: " + str(l) + "\n")
            #
            y_prob_test = agent.model.predict(x_test)
            #
            ################
            ...
            lower_thresh = np.percentile(pred_entropies, lower_percentile)
            upper_thresh = np.percentile(pred_entropies, upper_percentile)

            moderate_uncertainty_indices = np.where(
                (pred_entropies >= lower_thresh) & (pred_entropies <= upper_thresh)
            )[0]
            moderate_uncertainty_sorted = moderate_uncertainty_indices[
                np.argsort(np.array(pred_entropies)[moderate_uncertainty_indices])[::-1]
            ]
            #
            ...
            delete_indices = moderate_uncertainty_sorted[:AL]

            delete_indices = delete_indices.tolist()

            if len(explored_indices) == 0:
                #
                print('delete_indices', delete_indices)

                for k in range(len(delete_indices)):
                    #
                    explored_indices.append(delete_indices[k])
            else:
                #
                print('delete_indices', delete_indices)
                ...
                for k in range(len(delete_indices)):
                    ...
                    print('delete_indices[l]: ', delete_indices[k])
                    #
                    j = 1
                    #
                    while delete_indices[k] in explored_indices:
                        #
                        delete_indices[k] = moderate_uncertainty_sorted[AL + j]
                        #
                        print('resample delete_indices[l]: ', delete_indices[k])
                        #
                        j = j + 1
                    #
                    explored_indices.append(delete_indices[k])
            #
            print('explored_indices: ', explored_indices)
            #
            text_file.write('explored_indices: ' + str(explored_indices) + "\n")
            #
            ...
            for i in delete_indices:
                ...
                text_file.write('min_value: ' + str(min_value) + "\n")
                print('min_index: ', min_index)
                text_file.write('min_index: ' + str(min_index) + "\n")

                print('x_test[i]: ', x_test[i])

                print('x_test[[i]]: ', x_test[[i]])

                # print('y_train[min_index]: ', y_train[min_index])

                if np.array_equal(one_hot_y_pred_test[i], y_train[min_index]):
                    print("going to add to training set")
                    text_file.write("going to add to training set" + "\n")
                    print('x_train before: ', x_train)
                    print('y_train before: ', y_train)
                    x_train = np.vstack((x_train, x_test[[i]]))
                    y_train = np.vstack((y_train, one_hot_y_pred_test[i]))
                    print('x_train after: ', x_train)
                    print('y_train after: ', y_train)
                    agent.model.fit(x_test[[i]], y_train[[min_index]], epochs=1, verbose=0)

                else:
                    text_file.write('predicted label: ' + str(one_hot_y_pred_test[i]) + "\n")
                    print('predicted label: ', one_hot_y_pred_test[i])
                    print("going to relabel sample and then add to training set")
                    text_file.write("going to relabel sample and then add to training set" + "\n")
                    print('re-label: ', y_train[min_index])
                    text_file.write('using re-label: ' + str(y_train[min_index]) + "\n")
                    total_sample_relabel = total_sample_relabel + 1
                    x_train = np.vstack((x_train, x_test[[i]]))
                    y_train = np.vstack((y_train, y_train[min_index]))
                    agent.model.fit(x_test[[i]], y_train[[min_index]], epochs=1, verbose=0)

            print("================Test active learning==================")
            ...
            ###############################
            #if e == 4 and l == 2:
            #    print(f"Saving model at EPISODES = {e} and iter_AL = {l}")
            #    agent.model.save(model_save_path)
            #    text_file.write(f"Model saved at EPISODES = {e} and iter_AL = {l}\n")
            #    print("Model saved. Ending the loop.")
            #    break
        ...
        #if e == 4:  # After model is saved, break out of the outer EPISODES loop
        #    break
...
agent.model.load_weights(filepath = "weights_dqn_td_al_" + str(AL) + "_best.hdf5")
