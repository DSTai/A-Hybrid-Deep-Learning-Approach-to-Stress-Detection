...
def write_report_to_file(result, filePath, timePeriods, max_y_test, max_y_pred_test):
    ...
    #rp_dir = ("D:/Documents/THESIS/StressDetection-main/Model/reports/S16/result/wesad_resp_hybrid")
    rp_dir = os.path.join(project_root, "Model", "reports", "result", "wesad_resp_hybrid_less")
    os.makedirs(rp_dir, exist_ok=True)
    os.chdir(rp_dir)

    folder_name = os.path.splitext(all_filenames[0])[0]
...
def show_confusion_matrix(validations, predictions, timePeriods):
    ...
    plt.title('Confusion Matrix ACTIVITY', fontsize=14)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.ylabel('True Label', fontsize=12)
    plt.xticks(rotation=45, fontsize=10)
    plt.yticks(rotation=0, fontsize=10)
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{timePeriods}s.png", dpi=300)
    # plt.show()
...
def execute(TIME_PERIODS):
    ...
    argument_parser.add_argument("--train_file", type=str, default="/WESAD/train", help="Train file (CSV). Required for training.")
    argument_parser.add_argument("--validation_file", type=str, help="Validation file (CSV). Required for validation.")
    argument_parser.add_argument("--test_file", type=str, default="/WESAD/test", help="Test file (CSV). Required for testing.")
    ...
    # Add an LSTM layer
    model_m.add(layers.LSTM(128, return_sequences=True))
    model_m.add(layers.LSTM(128))
    model_m.add(layers.Dropout(0.5))

    # Add a Dense layer with softmax activation
    model_m.add(layers.Dense(128, activation='relu'))
    model_m.add(layers.Dense(num_classes, activation='softmax'))
    ...
    BATCH_SIZE = 64
    EPOCHS = 16
    report_dir = os.path.join(project_root, "Model", "reports", "result", "wesad_resp_hybrid_11subs", "S"+str(subject))
    os.makedirs(report_dir, exist_ok=True)
    WEIGHTS_PATH = os.path.join(project_root, "Model", "WESAD", "test")
    os.makedirs(WEIGHTS_PATH, exist_ok=True)
    os.makedirs(report_dir, exist_ok=True)

    print(f"Training for {EPOCHS} epochs")
        # serialize model to JSON
    filepath = os.path.join(WEIGHTS_PATH, f"weights_hybrid"+str(TIME_PERIODS)+str(STEP_DISTANCE)+str(
    EPOCHS) +"_activity.best.hdf5")
    ...
    model_m.load_weights("weights_hybrid"+str(TIME_PERIODS)+str(STEP_DISTANCE)+ str(EPOCHS) + "_activity.best.hdf5")
    ...
    report = (classification_report(max_y_test, max_y_pred_test))
    print(report)
    report_file_path = os.path.join(report_dir, f"{report_dir}/chest_resp_hybrid_batch_size{BATCH_SIZE}_epoch{EPOCHS}.txt")
    with open(report_file_path, "a") as text_file:
            text_file.write(f"Classification report for {EPOCHS} epochs:\n")
    ...
    show_confusion_matrix(max_y_test, max_y_pred_test, TIME_PERIODS)
